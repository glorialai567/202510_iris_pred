...
with tab1:
    for i, s in mapping.items():
        ...
        ax.scatter(subset['sepal length (cm)'],subset['sepal width (cm)'],label=i,c=colors[s])
...
with tab2:
    for i, s in mapping.items():
        subset=df[df['target']==s]
        ax2.scatter(subset['petal length (cm)'],subset['petal width (cm)'],label=i,c=colors[s])
